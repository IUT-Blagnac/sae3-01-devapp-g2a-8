<?php
    ...
    if (isset($_POST['email']) && isset($_POST['mdp']) && isset($_POST['continuer'])
        ...
        if (!preg_match("#[a-z]+#i", $_POST['prenom'])) {
            header("Location: formCreation.php?error=Format du prénom invalide");
            exit();
        }
        ...
        if (!preg_match("#^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[\#?!@/\\$%^&*}{)([\]~,;:_'-]).{12,}$#", $_POST['mdp'])) {
            header("Location: formCreation.php?error=Le mot de passe doit faire au moins 12 caractères, dont au moins un catactère spécial, une minuscule, une majuscule et un chiffre");
            exit();
        }
        ...
        if ($compte) {
            header("Location: formCreation.php?error=Cette adresse mail est déjà utilisée pour un autre compte");
            exit();
        }
