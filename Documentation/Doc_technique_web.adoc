...
[red]#&nbsp; &nbsp; &nbsp; &nbsp; La table utilisateur sera la plus manipulée. Elle contient toutes les informations personnelles d'un utilisateur, avec son mot de passe hashé et la propriété administrateur pour indiquer si il est bien administrateur du site. +
 +
&nbsp; &nbsp; &nbsp; &nbsp; La table Article contient toutes les informations d'un article en vente et possède un catégorie qui peut avoir un catégorie parente. Un article se décline en différentes couleurs. Ces articles ont donc un stock différent pour chaque variante de couleur. +
 +
&nbsp; &nbsp; &nbsp; &nbsp; Lors de son ajout au Panier, on enregistre également avec DetailPanier le nombre du même article dans le panier et aussi la variante du produit qui est ajouté. On associe donc à la commande uniquement le numéro du panier.# 
